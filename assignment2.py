...
# Python 2 / 3 compatibility for urllib2
try:
    import urllib2
except ImportError:
...
def setupLogger():
    """Configures the 'assignment2' logger to write log messages to errors.log strictly matching assignment format."""
    logger = logging.getLogger("assignment2")
    logger.setLevel(logging.ERROR)

    file_handler = logging.FileHandler("errors.log", mode="w")

    # Pure raw message formatting without extra timestamps or log levels
...
def downloadData(url):
    """Downloads content from the provided URL using urllib2."""
...
def main():
    ...
    # JetBrains / IDE Fallback: If no CLI arguments were passed, provide default test URL
    if len(sys.argv) == 1:
        sys.argv.extend(
            [
                "--url",
